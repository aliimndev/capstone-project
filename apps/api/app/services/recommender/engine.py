...
class InferenceEngine:
    def __init__(self, model_loader: ModelLoader, id_mapper: IDMapper) -> None:
        self.model_loader = model_loader
        self.id_mapper = id_mapper

    def _lookup_movie_index(self, movie_id: int) -> Optional[int]:
        if not self.model_loader.is_loaded or self.model_loader.art is None:
            return None
        movie_to_index = self.model_loader.art["movie_to_index"]
    ...
    def _build_movie_result(
        self, movie_id: int, source: str
    ) -> Optional[RecommendedMovie]:
        if not self.model_loader.is_loaded or self.model_loader.art is None:
            return None

        movies_df = self.model_loader.art["movies_filtered"]
        row_df = movies_df[movies_df["movieId"] == movie_id]
        if row_df.empty:
            return None

        row = row_df.iloc[0]
        tmdb_id = self.id_mapper.resolve_movie_to_tmdb_id(movie_id)
    ...
    def fill_from_top_trending(
        ...
        top_trending = self.model_loader.art["top_trending"]
        added = 0
        ...
        for _, row in top_trending.iterrows():
            ...
            item = RecommendedMovie(
                movieId=movie_id,
                tmdbId=self.id_mapper.resolve_movie_to_tmdb_id(movie_id),
                title=str(row["title"]),
                genres=str(row["genres"]) if pd.notna(row.get("genres")) else None,
    ...
    def get_standalone_top_trending_recommendations(
        ...
        results, _ = self.fill_from_top_trending(
            [],
    ...
    def debug_log_tmdb_lookups(self, raw_rated_movies: List[Dict[str, Any]]) -> None:
        ...
        for entry in raw_rated_movies:
            ...
            mapped_movie_id = self.id_mapper.lookup_tmdb_raw(movie_id_raw)
    ...
    def recommend_from_ratings(
        ...
        logger.info(
            "Starting recommendation generation for %d rated movies", len(rated_movies)
        )

        art = self.model_loader.art
        ...
        for item in rated_movies:
            ...
            movie_id = self.id_mapper.resolve_tmdb_to_movie_id(tmdb_id)
            mapping_log.append(
                {
                    "tmdbId": tmdb_id,
                    "movieId": movie_id,
        ...
        unmapped_tmdb_ids = [
            entry["tmdbId"] for entry in mapping_log if not entry.get("mapped")
        ]
        ...
        if len(results) < top_n:
            results, fallback_count = self.fill_from_top_trending(
                results,
